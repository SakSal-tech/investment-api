...
public class UserController {
	...
	public ResponseEntity<User> getUserByUsername(@PathVariable String username) {
        ...
        return ResponseEntity.ok(userService.getUserByUserName(username));
            ...
            //The .build() method is used with ResponseEntity to create a response without a body.	This returns a 404 Not Found response with no content.
		...
		for (int attempt = 1; attempt <= maxAttempts; attempt++) {
			try {
				...
				if (attempt == maxAttempts) {
					//After 3 failed login attempts, the controller now generates a reset token and returns it in the response body, 
					//so the user can use it to reset their password( I am using this to test on Postman. In reality this should be sent to user by email)
                    ...
                    //resetJson is a Java Map that holds information to send back to the client ( Potman) in JSON format.
	...
	public ResponseEntity<User> findByResetToken(@PathVariable String token) {
		...
		// Reset user password (now requires reset token)
		@PostMapping("/reset-password")
		public ResponseEntity<User> resetPassword(@RequestParam String username, @RequestParam String newPassword, @RequestParam String resetToken) {
			try {
				// Validate reset token before allowing password reset
				User user = userService.findByResetToken(resetToken);
				if (user == null || !user.getUsername().equals(username)) {
					return ResponseEntity.status(403).body(null); // Invalid token or username
				}
