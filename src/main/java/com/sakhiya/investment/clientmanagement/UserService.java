...
public class UserService {
    ...
    public User getUserByUserName(String userName) throws NoSuchElementException{
        return userRepository.findByUsername(userName)
            ...
            .orElseThrow(() -> new NoSuchElementException("User with this userName: " + userName + " is not found"));
    }
    ...
    /**
     * This method accepts 2 parameters but either one is accepted. Users could be found by username or email.
     */
    public User getUserByEmailOrUsername(String userName, String email) throws NoSuchElementException {
        // validation if either email or userName only is inputted to call other methods findByUsername and findByEmail 
     ...
     * If the user is inactive or locked out, this method does not allow login and throws an exception.
     * Throws NoSuchElementException if the user is not found or credentials are invalid.
     * Note: Handling for locked out users is not implemented here; only active users can log in.
     *
    ...
    public User userLogin(String userName, String rawPassword) {
        ...
        User user = userRepository.findByUsername(userName)
            .orElseThrow(() -> new NoSuchElementException("User with this username: " + userName + " is not found"));
        //encoder.matches is a method from BCryptPasswordEncoder that checks if a raw (plain text) password matches a previously hashed password.
         if (encoder.matches(rawPassword, user.getPasswordHash())) {
            return user; // login successful
        }
        // If password does not match, handle failed login attempts (not implemented here)
        throw new NoSuchElementException("Invalid credentials. Reset your password.");
    }
    //This method generates a unique, random token (usually a UUID) and stores it in the user's record.
    //It is used when a user requests a password reset (e.g., “Forgot Password?”).
    // In future improved system, the plan is that token is sent to the user (usually via email) as a link to verify their identity and allow them to reset their password.
    public User generateResetToken(String userName) {
        // find user to to generate a reset token  by creating a user object and calling the generate method
        User user = userRepository.findByUsername(userName).orElseThrow(() -> new NoSuchElementException("User with this username: " + userName + " is not found"));
        user.generateResetToken();
        // To save)the new token value to the database field, calling save on the repository
     ...
     * Used during password reset flows to validate and retrieve the user associated with a reset token.
     */
    ...
    /**
     * Creates a new user. Throws IllegalArgumentException if username or email already exists.
     */
